...
image_base = Path('/home/leannenortje/Datasets/spokencoco')
support_set = np.load(ss_save_fn, allow_pickle=True)['support_set'].item()
files = "/home/leannenortje/Datasets/spokencoco/SpokenCOCO"
...
for id in query_scores:
    ...
    if id not in record: record[id] = []
    indices = np.argsort(query_scores[id]['values'])[::-1]

    for i in range(top_N):
        img =  Path(query_scores[id]['imgs'][indices[i]])
        img_name = img.stem
        record[id].append((i, img, query_scores[id]['values'][indices[i]]))

np.savez_compressed(
    Path("../data/sampled_img_ranks_and_scores"), 
    record=record
)